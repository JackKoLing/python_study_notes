...
def svae_high_score(stats):
    """ 退出游戏时保存最高分 """
    high_score = int(round(stats.high_score, -1)) # -1将其规整到10数倍
...
def check_keydown_events(event, ai_settings, stats, screen, ship, bullets):
    ...
    if event.key == pygame.K_RIGHT:
        ship.moving_right = True # 为了实现不断移动
    elif event.key == pygame.K_LEFT:
        ship.moving_left = True
    elif event.key == pygame.K_SPACE:
        fire_bullet(ai_settings, screen, ship, bullets)
    elif event.key == pygame.K_q:
        svae_high_score(stats)
        sys.exit()
...
def check_keyup_events(event, ship):
    """ 响应松开事件 """
    if event.key == pygame.K_RIGHT:
        ship.moving_right = False # 为了实现不断移动
    elif event.key == pygame.K_LEFT:
...
def check_high_score(stats, sb):
    ...
    if stats.score > stats.high_score:
        ...
        sb.prep_high_score() # 更新最高分图像

def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, 
                                aliens, bullets):
    """ 响应子弹和外星人的碰撞 """
    # 检查是否有子弹击中了外星人
    # 如果是，则删除相应的子弹和外星人,用后两个形参True表示
